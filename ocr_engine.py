...
def predict_digit(rect,img_thrsh,clf):
    ...
    #resize the region of interest to 20*20
    roi=cv2.resize(roi,(20,20),interrpolation=cv2.INTER_AREA)#inter area is for interpolation
    roi=cv2.dilate(roi,(3,3))
    
    #calculate the HOG features to predict the number
    features=hog(roi,pixels_per_cell=(10,10),cells_per_block=(1,1))
    #print(features)
    #predict the number
...
def perform_ocr(img,clf,write=True):
    #read teh file and perform some image processing
    img_gray=cv2.cvtColor(img,cv2.COLOR_RGB2GRAY)
    img_gray_blur=cv2.GaussianBlur(img_gray,(5,5),0)
    _,img_thrsh=cv2.threshold(img_gray_blur,90,255,cv2.THRESH_BINARY_INV)
    
    #find the contours of the digits
    image,contours=cv2.findContours(img_thrsh,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
